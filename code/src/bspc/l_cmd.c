...
//	vsprintf( szBuffer, szFormat, argptr );
	Q_vsnprintf( szBuffer, sizeof(szBuffer), szFormat, argptr );	// Knightmare- buffer overflow fix
	va_end( argptr );
...
//	vsprintf( text, warning, argptr );
	Q_vsnprintf( text, sizeof(text), warning, argptr );	// Knightmare- buffer overflow fix
	va_end( argptr );
	printf( "WARNING: %s\n", text );
